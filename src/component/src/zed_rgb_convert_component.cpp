...
: Node("zed_cvt_node", options), mVideoQos(1)
{
  ...
  mPubBgr_right = image_transport::create_camera_publisher(
    this, "~/zed_image_3ch_right", mVideoQos.get_rmw_qos_profile());
  RCLCPP_INFO_STREAM(get_logger(), "Advertised on topic: " << mPubBgr_right.getTopic());

  // Create camera subscriber
  mSubBgra_right = image_transport::create_camera_subscription(
    this, "zed_image_4ch_right", std::bind(&ZedRgbCvtComponent::right_camera_callback, this, _1, _2), "raw",
    mVideoQos.get_rmw_qos_profile());
...
void ZedRgbCvtComponent::right_camera_callback(
  const sensor_msgs::msg::Image::ConstSharedPtr & img,
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & cam_info)
{
  // Check for correct input image encoding
  ...
  // Publish the new image message coupled with camera info from the original message
  mPubBgr_right.publish(out_bgr, cam_info);
}
}  // namespace stereolabs
